...
public class DbSchemaExporter {
    ...
    public static void main(String[] args) {
        try {
            if (args.length < 2) {
                System.out
                        .println("Usage: DbSchemaExporter <spring-configuration-classname> <output-filename>");
                System.exit(1);
            }
            String springConfigClassname = args[0];
            String filename = args[1];
            exportSchemaToFile(springConfigClassname, filename);
        } catch (Exception all) {
