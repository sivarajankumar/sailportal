 ...
 *
 * @author Ben Alex
 * @version $Id: BasicLookupStrategy.java,v 1.5 2007/02/27 20:16:02 simonv Exp $
 */
public final class BasicLookupStrategy implements LookupStrategy {
    ...
    private Class objectIdentifierClass = java.lang.Long.class;
    private String objectIdentifierColumnName = "object_id_identity_num";
...
/**
    ...
    private String computeRepeatingSql(String repeatingSql, int requiredRepetitions) {
        ...
        String startSql = "select ACL_OBJECT_IDENTITY." + this.getObjectIdentifierColumnName() + ", " 
          + "ACL_ENTRY.ACE_ORDER,  "
        ...
        String endSql = ") order by ACL_OBJECT_IDENTITY." 
          + this.getObjectIdentifierColumnName() 
    ...
    private void convertCurrentResultIntoObject(Map acls, ResultSet rs)
        ...
        if (acl == null) {
          ...
          Serializable domainId = this.getObjectIdFromResultSet(rs);
            ObjectIdentity objectIdentity = new ObjectIdentityImpl(rs.getString("CLASS"), domainId);
    ...
    private Map lookupObjectIdentities(final ObjectIdentity[] objectIdentities, Sid[] sids) {
        ...
        String sql = computeRepeatingSql("(ACL_OBJECT_IDENTITY." + this.getObjectIdentifierColumnName() + " = ? and ACL_CLASS.CLASS = ?)",
            ...
            new PreparedStatementSetter() {
                public void setValues(PreparedStatement ps)
                    ...
                    for (int i = 0; i < objectIdentities.length; i++) {
                        ...
                        Assert.isInstanceOf(getObjectIdentifierClass(), objectIdentities[i].getIdentifier(),
                            "This class requires ObjectIdentity.getIdentifier() to be a " + getObjectIdentifierClass().getName());

//                        long id = ((Long) objectIdentities[i].getIdentifier()).longValue();
                        Serializable id = objectIdentities[i].getIdentifier();

                        // Inject values
//                        ps.setLong((2 * i) + 1, id);
                        ps.setObject((2 * i) + 1, id);
    ...
    /**
     * Obtains the Serializable Object ID from a result set as a serializbale,
     * the exact class of the id is dependent on the values of the member variable
     * <code>objectIdentifierClass</code>.  The column of the result set it is 
     * extracted from is dependent on the value of the member variable 
     * <code>objectIdentifierColumnName</code>.  This method does not close the
     * result set.
     * @param rs The ResultSet from which to extract the value, cannot be null
     * @return The object identifier as a Serializable
     * @throws SQLException
     */
    public Serializable getObjectIdFromResultSet(ResultSet rs) throws SQLException {
      Assert.notNull(rs);
      
      Serializable objectId = null;
      if (Integer.class.equals(this.getObjectIdentifierClass())) {
        objectId = new Integer(rs.getInt(this.getObjectIdentifierColumnName()));
      }
      else if (Long.class.equals(this.getObjectIdentifierClass())) {
        objectId = new Long(rs.getLong(this.getObjectIdentifierColumnName()));
      }
      else if (String.class.equals(this.getObjectIdentifierClass())) {
        objectId = rs.getString(this.getObjectIdentifierColumnName());
      }
      else {
        throw new IllegalStateException("Dont know how to get Object ID's of class " + objectIdentifierClass);
      }
    ...
    /**
     * @see LookupStrategy.getObjectIdentifierClass()
     */
    ...
    /**
     * Sets the configured class of the expected return value from 
     * <code>ObjectIdentity.getIdentifier()</code>.  If not set the default 
     * value is <code>java.lang.Long</code>.   
     * @param objectIdentifierClass The class of object returned by 
     * <code>ObjectIdentity.getIdentifier()</code>.  
     */
    public void setObjectIdentifierClass(Class objectIdentifierClass) {
      Assert.notNull(objectIdentifierClass);
      this.objectIdentifierClass = objectIdentifierClass;
    }
        
    
    /**
     * @see LookupStrategy.getObjectIdentifierColumnName()
     */
    public String getObjectIdentifierColumnName() {
      return objectIdentifierColumnName;
    }

    /**
     * Sets the name of the column that contains the objectIdentifier in the 
     * ACL_OBJECT_IDENTITY.  The default is OBJECT_ID_IDENTITY_NUM, suitable
     * for objectIdentities of Long and Integer, another option is 
     * OBJECT_ID_IDENTITY_CHAR suitable for Strings, a possible 3rd option which 
     * is not enabled by default is OBJECT_ID_IDENTITY_UUID suitable for 
     * UUID.  In the GUID case objectIdentifierClass would be String.  You'll 
     * also need to check RDBMS support for UUID
     * @param objectIdentifierColumnName The column name
     */
    public void setObjectIdentifierColumnName(String objectIdentifierColumnName){
      Assert.hasText(objectIdentifierColumnName);
      this.objectIdentifierColumnName = objectIdentifierColumnName;
    }
}
