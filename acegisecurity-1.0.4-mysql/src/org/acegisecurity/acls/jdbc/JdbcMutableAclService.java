 ...
 * @version $Id: JdbcMutableAclService.java,v 1.3 2007/02/26 22:08:30 simonv Exp $
 */
public class JdbcMutableAclService extends JdbcAclService implements MutableAclService {
    ...
    /**
     * The identity query used to retrieve database generated ID calues on insert.
     * Will vary between database systems. Should be be injected via Spring 
     * configuration.
     */
    ...
    public void afterPropertiesSet() {
      ...
      insertObjectIdentity = "INSERT INTO acl_object_identity "
        + "(object_id_class, object_id_identity, " + lookupStrategy.getObjectIdentifierColumnName() + ", owner_sid, entries_inheriting) " + "VALUES (?, ?, ?, ?, ?)";
      
      
      selectObjectIdentityPrimaryKey = "SELECT acl_object_identity.id FROM acl_object_identity, acl_class "
        + "WHERE acl_object_identity.object_id_class = acl_class.id and acl_class.class=? "
        + "and acl_object_identity." + lookupStrategy.getObjectIdentifierColumnName() + " = ?";
    }
...
System.out.println("objectId:"+object.getIdentifier().toString());
System.out.println("sid"+sidId);
        jdbcTemplate.update(insertObjectIdentity,
            new Object[] {classId, object.getIdentifier().toString(), object.getIdentifier().toString(), sidId, new Boolean(true)});
    }
    ...
    /**
     * Getter for the String containing the SQL incantation of the 
     * identity query.
     * @return A <Code>String</code> containing the identity query.
     */
    public String getIdentityQuery()
    {
      if (identityQuery == null || identityQuery.trim().length() ==0) {
        throw new IllegalStateException("Identity query has not been set, or has been set to the empty String");
      }
      return identityQuery;
    }

    /**
     * Setter for the identity query. For example if using an hsql
     * data-base, this would be: call identity(), if using MS SQL server it 
     * would be: select @@identity
     * @param identityQuery
     */
    public void setIdentityQuery(String identityQuery)
    {
      Assert.hasText(identityQuery);
